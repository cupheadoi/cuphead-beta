...
export function applyMigrationsAndSchema() {
  ...
  // Older releases stored recoverable passwords. Destroy those values immediately;
  // authentication only needs the scrypt hash.
  try {
    const hasLegacyPlaintextColumn = db.prepare('PRAGMA table_info(users)').all()
      .some(column => column.name === 'plain_password');
    if (hasLegacyPlaintextColumn) {
      db.exec("UPDATE users SET plain_password='' WHERE plain_password<>''");
      try { db.exec('ALTER TABLE users DROP COLUMN plain_password'); } catch {}
    }
  } catch (error) {
    console.warn('Could not clear the legacy plaintext password column:', error);
  }
}
...
export function validateDatabaseFile(filePath) {
  ...
  try {
    ...
    const required = candidate.prepare("SELECT COUNT(*) AS count FROM sqlite_master WHERE type='table' AND name IN ('users','problems','lessons')").get();
    if (Number(required?.count) !== 3) throw new Error('Required CupHead tables are missing.');
  } finally {
    candidate.close();
  }
  return true;
