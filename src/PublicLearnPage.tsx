...
import type { PublicBootstrap, Rank, Section } from "./types";
...
export default function PublicLearnPage() {
  ...
  const lessonCount = useMemo(() => modules.reduce((count, module) => count + module.lessonIds.length, 0), [modules]);
      ...
      <motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .35 }} className="glass-card relative overflow-hidden rounded-[1.75rem] p-6 sm:p-9">
        ...
        <div className="relative grid gap-7 lg:grid-cols-[1fr_auto] lg:items-end">
          ...
          <div className="grid grid-cols-2 gap-3 sm:min-w-[16rem]">
            <div className="rounded-2xl border border-slate-400/15 bg-slate-950/25 p-4"><Layers3 className="text-cyan-200" size={19} /><div className="mt-5 text-2xl font-black text-white">{modules.length}</div><div className="mt-1 text-xs text-slate-500">ماژول آموزشی</div></div>
