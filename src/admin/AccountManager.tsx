...
export default function AccountManager({ user }: { user: SessionUser }) {
  ...
  function isPasswordVisible(id: string) {
    if (visibleMap[id] !== undefined) {
      return visibleMap[id];
    }
    ...
    <div>
      ...
      <div className="glass-card rounded-2xl divide-y divide-slate-800/80 border border-slate-800">
        {filtered.map(account => {
          const visible = isPasswordVisible(account.id);
          ...
          return (
            <div key={account.id} className="p-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center gap-2.5">
                {/* Password Box */}
                <div className="flex items-center gap-2 bg-slate-900/90 border border-slate-700/70 px-3 py-1.5 rounded-xl text-xs">
                  <Lock size={13} className="text-cyan-400 shrink-0" />
                  <span className="text-slate-400 text-[11px]">رمز:</span>
                  <span
                    className={`font-mono text-cyan-200 select-all ${visible ? 'font-semibold tracking-normal' : 'tracking-widest'}`}
                    dir="ltr"
                  >
                  ...
                  {account.password && (
                    <button
                      ...
                      title="کپی رمز"
                    >
                      {copiedId === account.id ? <Check size={13} className="text-emerald-400" /> : <Copy size={13} />}
                    </button>
                  )}
                </div>
          ...
          <div className="glass-card rounded-3xl p-6 w-full max-w-md border border-cyan-400/30">
            ...
            <form onSubmit={submitPasswordChange} className="space-y-4">
              <div className="p-3 rounded-xl bg-slate-900/80 border border-slate-800 text-xs space-y-1">
                <div className="text-slate-400">کاربر: <span className="text-white font-bold">{passwordModalUser.displayName}</span></div>
                <div className="text-slate-400">رمز عبور فعلی: <span className="font-mono text-cyan-300" dir="ltr">{passwordModalUser.password || 'تعیین نشده'}</span></div>
              </div>
