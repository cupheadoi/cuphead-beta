...
export default function CodeBlock({code,language='cpp'}:{code:string;language?:string}){
 ...
 return <div className="relative rounded-2xl overflow-hidden border border-dark-border shadow-2xl" dir="ltr">
  ...
  <SyntaxHighlighter language={language} style={vscDarkPlus} showLineNumbers customStyle={{margin:0,padding:'1rem',background:'#0d111a',fontFamily:'"JetBrains Mono", monospace',fontSize:'14px',lineHeight:'1.7'}} lineNumberStyle={{minWidth:'2.5em',paddingRight:'1em',color:'#4b5563',textAlign:'right'}}>{code.trim()}</SyntaxHighlighter>
 </div>
}
